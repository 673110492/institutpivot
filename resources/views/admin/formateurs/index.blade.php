    ...
    <div class="card-body position-relative">
        <div class="row">
            <div class="col-lg-8">
                ...
                <p class="mb-0"><a href="{{ url('/home') }}">Dashboard</a> / Liste des formateurs</p>
            </div>
            <br>
            <div class="col-12">
                @if ($errors->any())
...
<div class="card">
                            ...
                            <div class="card-body p-0 pb-3">
                                <div class="d-flex align-items-center justify-content-end my-3">
                                    <div id="bulk-select-replace-element" style="margin-right: 20px">
                                        <button title="Ajout d'un formateur" data-bs-toggle="modal" data-bs-target="#ajoutFormateur" class="btn btn-falcon-success btn-sm">
                                        ...
                                        <table class="table table-striped mb-0 fs--3 overflow-hidden">
                                            <thead class="bg-200 text-900">
                                                <tr>
                                                    ...
                                                    <th class="align-middle" data-sort="adresse">Adresse</th>
                                                    <th class="align-middle" data-sort="sexe">Sexe</th>
                                                    <th class="align-middle" data-sort="tel">Telephone</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody class="list" id="table-orders-body">
                                                @forelse ( $formateurs as $item)
                                                <tr class="btn-reveal-trigger">
                                                    ...
                                                    <th class="align-middle nom">{{ $item->noms }}</th>
                                                    <td class="align-middle email">{{$item->email}}</td>
                                                    <td class="align-middle adresse">{{ $item->adresse }}</td>
                                                    <td class="align-middle sexe">{{ $item->sexe }}</td>
                                                    <td class="align-middle tel">{{ $item->tel1 }}</td>
                                                    <td class="align-middle mr-20">
                                                        <button class="btn btn-falcon-info me-1 mb-1"
                                                            title="Détail d'un formateur" data-bs-toggle="modal"
                                                            data-bs-target="#staticBackdrops{{ $item->id }}" type="button"><i
                                                                class="fa fa-eye"></i></button>
                                                        <button class="btn btn-falcon-primary me-1 mb-1" title="Modifier un abonnement" data-bs-toggle="modal"
                                                                data-bs-target="#editFormateur{{ $item->id }}"
                                                                type="button"><i class="fa fa-pen"></i>
                                                        </button>
                                                        <button class="btn btn-falcon-danger me-1 mb-1" title="Supprimer un formatuer"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#{{ 'delete' . $item->id }}"><i class="fa fa-trash"
                                                                aria-hidden="true"></i></button>
                                                        @include('global.delete-modal', [
                                                        'id' => $item->id,
                                                        'url' => route('formateur.delete', $item->id),
                                                        ])
                                                        @include('admin.formateurs.edit', [
                                                            'formateur' => $item,
                                                        ])
                                                    </td>
                                                </tr>
    ...
    {{-- MODAL CREATE --}}
    @include('admin.formateurs.create')
@endsection
